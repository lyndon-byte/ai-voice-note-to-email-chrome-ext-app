...
export default function EmailScreen() {
  ...
  const insertIntoGmailStream = async (subjectText, bodyText, isDone = false) => {
    try {
      ...
      if (!tab?.id) {
        console.log('[Gmail Inject] No active tab found');
        return;
      }
      ...
      await chrome.scripting.executeScript({
        ...
        func: (subj, body, done) => {
          ...
          console.log('[Gmail Inject] Subject visible:', subjectVisible, '| Body visible:', bodyVisible);

          // ── Remove toast on done ────────────────────────────────────────
          if (done) {
          ...
          if (!done) {
            ...
            if (fieldsEl) {
              fieldsEl.innerHTML = fields
                .map(f => '<span class="__tag__">' + f + '</span>')
                .join('');
            }
          }
